...
- использует XOR по умолчанию для совместимости с NexaTunnel на iOS;
...
Новая установка использует `XOR` для совместимости с NexaTunnel на iOS:
...
> [!WARNING]
> XOR обеспечивает только сокрытие данных и не является криптографически
> безопасным шифрованием. Если все ваши клиенты поддерживают ChaCha20 или AES,
> выберите более сильный метод явно. Изменение значения по умолчанию применяется
> только к новым установкам; существующая серверная конфигурация сохраняется.
...
| `xor` | 1 | значение по умолчанию для совместимости с NexaTunnel; криптографически небезопасно |
| `chacha20` | 2 | более сильный метод для совместимых клиентов |
| `aes-128-gcm` | 3 | AEAD |
| `aes-192-gcm` | 4 | AEAD |
| `aes-256-gcm` | 5 | AEAD |
